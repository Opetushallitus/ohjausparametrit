...
public class SiirtotiedostoAjastusService {
  ...
  public SiirtotiedostoAjastusService(
      ...
      SiirtotiedostoProperties siirtotiedostoProperties) {
  ...
  public String createNextSiirtotiedosto() {
    ...
    try {
      ...
      while (!dbResults.isEmpty()) {
        ...
        logger.info(
            "{} Tallennetaan {} tulosta, sivu {}", uusi.getExecutionUuid(), dbResults.size(), page);
        keys.add(
            siirtotiedostoService.createSiirtotiedosto(dbResults, uusi.getExecutionUuid(), page));
        dbResults =
            parameterService.getPartitionByModifyDatetime(
                Date.from(uusi.getWindowStart().toInstant()),
                Date.from(uusi.getWindowEnd().toInstant()),
